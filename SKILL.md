...
2. **`shazam_overview` before touching any new repo.** It shows the spine of the codebase in one call.
3. **`shazam_codequery --symbol` before editing any function/class.** Verify it exists and see its callers.
...
|-----------|------|-------|
| First entry into any repo | `shazam_overview` | Module map, top-10 PageRank files, reading order, hotspots |
...
| Single symbol lookup | `shazam_symbol --symbol <name>` | Definition, visibility, signature, call counts |
...
| HTTP route inventory | `shazam_routes` | All HTTP routes/methods/handlers discovered from the codebase |
| State machine map | `shazam_state_map` | Enum/state definitions + transitions |
| Dead code detection | `shazam_orphan` | Symbols with zero incoming references (≥70 confidence) |
