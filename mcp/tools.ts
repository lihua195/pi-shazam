...
export function registerAllTools(server: McpServer, graph: RepoGraph, projectRoot: string): void {
	...
	server.registerTool("shazam_find_tests", {
		...
		withLogging("shazam_find_tests", async ({ sourceFile, module: mod }) => {
			const result = executeFindTests(graph, projectRoot, { sourceFile: sourceFile as string | undefined, module: mod as string | undefined });
	...
	server.registerTool("shazam_type_hierarchy", {
		...
		withLogging("shazam_type_hierarchy", async ({ name, direction }) => {
			const result = executeTypeHierarchy(graph, name as string, (direction as "both" | "supertypes" | "subtypes") ?? "both");
			return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
		}),
	);
